...
runfile('/home/dominik/projects/python/vm_data/src/appendCellData.py', 
        ...
        args='ys_quad_hex_reordered.vtu ys_domain_appended_cell_data.vtu')
...
#Implementation of multiple material properties to mesh of known size
Ra_size = np.random.randint(10,100001,cells_count)
mat_size = int((k_max - k_min)/3)
Ra_cell = np.zeros(np.shape(k_cell))
for n in range(0,9):
    for i in range(n*mat_size,n*mat_size+mat_size):
        Ra_cell = np.random(10,10000,1)
