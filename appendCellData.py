...
nnn = np.array([32 ,32,0],'i')
xyz = np.array([0,32,0,32,0,0],'f')
w   = np.zeros([32,32,0])
...
cells_count = len(mesh.cells[0])
centerpoints=np.zeros((cells_count, 3))
for cellblock in mesh.cells:
    print(cellblock.index)
    #centerpoints[cellblock.index] = np.sum(mesh.points[cellblock], axis=0) / len(cellblock)
